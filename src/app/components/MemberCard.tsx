import {calendarSignRebate, hasCalendarSignRebate, queryUserActivityAccount, queryUserCreditAccount} from "@/apis";
...
export function MemberCard({allRefresh}) {
    ...
    const hasCalendarSignRebateHandle = async () => {

        const queryParams = new URLSearchParams(window.location.search);
        const result = await hasCalendarSignRebate(String(queryParams.get('userId')));
    ...
    useEffect(() => {
        ...
        hasCalendarSignRebateHandle().then(r => {
        });
    }, [refresh, allRefresh])
